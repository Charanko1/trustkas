...
export default function OrganizationPage() {
  ...
  async function fetchData(showLoading = true) {
    ...
    try {
      ...
      const [o, g, m, r, p] = await Promise.all([
      ...
      const requestData = r ? await r.json() : [];
      ...
      const me = list.find((x: Member) => x.name === profile.name);
  ...
  return (
    <>
      <JoinRequestModal
        ...
        requests={requests}
        onApprove={(id) => api(`/api/groups/join-requests/${id}`, { method: "PATCH", body: JSON.stringify({ action: "Approved" }) })}
        onReject={(id) => api(`/api/groups/join-requests/${id}`, { method: "PATCH", body: JSON.stringify({ action: "Rejected" }) })}
      />

      <CreateGroupModal
        open={groupOpen}
        onClose={() => setGroupOpen(false)}
        onCreate={(g) => api("/api/groups", { method: "POST", body: JSON.stringify({ ...g, organizationSlug: orgId }) })}
      />
      ...
      <div className="space-y-6">
        <div className="relative">
          ...
          <OrganizationMenu
            ...
            code={org.code}
            copyCode={copyCode}
            exitOrganization={() => api("/api/organizations/exit", { method: "DELETE", body: JSON.stringify({ organizationId: org._id }) })}
            deleteOrganization={() => api(`/api/organizations/${org._id}`, { method: "DELETE" })}
          />
        </div>
        ...
        <MemberList
          members={members}
          myRole={myRole}
          onSetValidator={(id) => api(`/api/memberships/${id}`, { method: "PATCH", body: JSON.stringify({ role: "Validator" }) })}
          onRemove={(id) => api(`/api/memberships/${id}`, { method: "DELETE" })}
        />
        ...
        <GroupList
          ...
          onJoin={(id) => api(`/api/groups/${id}/join`, { method: "POST" })}
        />
      </div>
    </>
  );
